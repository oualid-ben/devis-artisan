...
const FormPage = () => {
  ...
  const [searchParams, setSearchParams] = useSearchParams();
    ...
    let i = 1;
    while (params[`prestation${i}`]) {
        prestations.push({
        typedeprestation: params[`prestation${i}`],
        quantite: {
            valeur: parseFloat(params[`qte${i}`]) || 1,
            unite: params[`unite${i}`] || 'u',
        },
        prixunitaire: parseFloat(params[`pu${i}`]) || 0,
        tauxtva: params[`tva${i}`]?.trim() !== "" ? parseFloat(params[`tva${i}`]) : 20
        });
        i++;
    }
    ...
    setData(prev => ({
        ...prev,
        clientNom: params.clientNom || prev.clientNom,
